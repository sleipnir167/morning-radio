...
| profile | 目次 | 本文 | 結合 | 使う枠 | 1日の回数 |
|---|---|---|---|---|---|
| （定時実行・`high`） | 3.6 Flash | 3.6 Flash | 3.6 Flash | 3.6の20 | 2回 |
| `standard`（手動の既定） | 3.5 Flash | 3.5 Flash | 3.5 Flash | 3.5の20 | 2回 |
| `light` | 3.5 Flash | 3.5 Flash Lite | 3.5 Flash | Liteの500 | 50回 |

枠がモデルごとに独立しているので、`standard` は 3.6 Flash の枠を1回も使わない。
**毎朝の1本を最上位モデルで確保したまま、日中にもう2本作れる**のがこの配分の狙い。

`light` は本文だけを Lite に逃がすが、実測では中身の密度が落ちる
（同じ回で年号の登場が25箇所→1箇所、平均文長が41.7字→51.7字）。
数をこなしたいときや、ジャンルを試したいときの下見用と考えたほうがよい。
...
- **Gemini が 503 を返す**: `This model is currently experiencing high demand` は
  そのモデルが混んでいるだけなので、30秒・60秒と待ったうえで `llm.fallback_models` の
  次のモデルへ切り替えて続行する。一度切り替えたらその回は最後まで同じモデルを使う
  （章ごとに語り口が変わるのを避けるため）。それでも駄目なら5分おいて全体を1回やり直す。
- **Gemini のレート制限**: 429が返ったら、APIが指定してきた待ち時間だけ待って自動で再試行する
  （`llm.rate_limit_retries` 回まで）。ただし**日あたりの上限**に当たった場合は待っても解除されない。
  1回の放送で10リクエスト前後を使うので、無料枠の残りは https://ai.dev/rate-limit で確認すること。
  1日に何度も作り直したいなら `llm.provider` を `openrouter` にするのが確実。
