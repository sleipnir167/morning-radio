...
- **Gemini のレート制限**: 無料枠は分あたりのリクエスト数に上限がある（`gemini-3.6-flash` で20件）。
  429が返ったら、APIが指定してきた待ち時間だけ待ってから自動で再試行する（`llm.rate_limit_retries` 回まで）。
  短時間に何回も手動実行を重ねると待ちが増えるので、続けて回すなら数分空けるとよい。
